...
if($('#sales_charts').length > 0) {
  ...
  $(document).ready(function () {
    if ($('#leadpiechart').length > 0) {
        $.ajax({
            url: "Home/getLeadsBySource",  // Ensure this URL returns the correct data
            method: "GET",
            dataType: "json",
            success: function (response) {
                if (response.data.length > 0) { // Ensure data exists
                    var options = {
                        series: response.data,
                        chart: { width: 400, type: 'pie' },
                        labels: response.labels,  // These should be the names of the lead sources
                    ...
                    $("#leadpiechart").html("<p>No data available</p>"); // Show message if empty
                }
            },
            error: function () {
                console.log("Error fetching data");
                $("#leadpiechart").html("<p>Error loading chart</p>");
            }
        });
    }
});
